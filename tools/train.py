...
def parse_args():
    ...
    parser.add_argument(
        "--method", dest="method", help="set the method to use", default="CAN", type=str
    )
    parser.add_argument(
        "--exp_name",
        dest="exp_name",
...
def train(args):
    ...
    if args.method == "CAN":
        ...
        raise NotImplementedError(
            "Currently don't support the specified method: %s." % args.method
        )
    ...
    # initialize solver
    train_solver = Solver(
        net, dataloaders, bn_domain_map=bn_domain_map, resume=resume_dict
    )
