...
class CDD(object):
    ...
    def patch_gamma_estimation(self, nums_S, nums_T, dist):
        ...
        for ns in range(num_classes):
            ...
            for nt in range(num_classes):
                ...
                patch["st"] = (
                    dist["st"]
                    .narrow(0, source_start, nums_S[ns])
                    .narrow(1, target_start, nums_T[nt])
                )
    ...
    def compute_kernel_dist(self, dist, gamma, kernel_num, kernel_mul):
        ...
        eps = 1e-5
        gamma_mask = (gamma_tensor < eps).type(torch.FloatTensor)  # cuda
        ...
        upper_mask = (dist > 1e5).type(torch.FloatTensor).detach()  # cuda
        lower_mask = (dist < 1e-5).type(torch.FloatTensor).detach()  # cuda
    ...
    def kernel_layer_aggregation(self, dist_layers, gamma_layers, key, category=None):
        ...
        for i in range(num_layers):
            ...
            dist = (
                dist_layers[i][key]
                if category is None
                else dist_layers[i][key][category]
            )

            gamma = (
                gamma_layers[i][key]
                if category is None
                else gamma_layers[i][key][category]
            )
            ...
            if kernel_dist is None:
                kernel_dist = self.compute_kernel_dist(
                    dist, gamma, cur_kernel_num, cur_kernel_mul
                )
    ...
    def patch_mean(self, nums_row, nums_col, dist):
        ...
        for row in range(num_classes):
            ...
            for col in range(num_classes):
                ...
                val = torch.mean(
                    dist.narrow(0, row_start, nums_row[row]).narrow(
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        for c in range(num_classes):
            kernel_dist_ss += [
                torch.mean(
                    self.kernel_layer_aggregation(
                        dist_layers, gamma_layers, "ss", c
                    ).view(num_classes, -1),
                    dim=1,
                )
            ]
        ...
        if not self.intra_only:
            inter_mask = to_cuda(
                (torch.ones([num_classes, num_classes]) - torch.eye(num_classes)).type(
                    torch.ByteTensor
                )
            )
