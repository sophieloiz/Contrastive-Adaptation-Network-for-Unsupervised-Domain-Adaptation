...
sys.path.insert(
    1,
    "/Users/sophie.loizillon/gitlab/Contrastive-Adaptation-Network-for-Unsupervised-Domain-Adaptation/",
)
from . import resnet
...
class DANet(nn.Module):
    def __init__(
        ...
        self.feature_extractor = find_class_by_name(feature_extractor, backbones)(
            pretrained=fx_pretrained, frozen=frozen, num_domains=num_domains_bn
        )
        ...
        cur_dropout_ratio = (
            dropout_ratio[self.num_hidden_layer]
            if self.num_hidden_layer < len(dropout_ratio)
            else 0.0
        )
