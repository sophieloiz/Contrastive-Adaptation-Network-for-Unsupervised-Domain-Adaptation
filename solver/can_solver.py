...
class CANSolver(BaseSolver):
    def __init__(self, net, dataloader, bn_domain_map={}, resume=None, **kwargs):
        super(CANSolver, self).__init__(
            net, dataloader, bn_domain_map=bn_domain_map, resume=resume, **kwargs
        )
        ...
        self.cdd = CDD(
            kernel_num=self.opt.CDD.KERNEL_NUM,
    ...
    def complete_training(self):
        ...
        if (
            "target_centers" not in self.history
            or "ts_center_dist" not in self.history
            or "target_labels" not in self.history
        ):
            return False

        if (
            len(self.history["target_centers"]) < 2
            or len(self.history["ts_center_dist"]) < 1
            or len(self.history["target_labels"]) < 2
        ):
            return False
        ...
        eval1 = torch.mean(
            self.clustering.Dist.get_dist(target_centers[-1], target_centers[-2])
        ).item()
        ...
        return (
            eval1 < self.opt.TRAIN.STOP_THRESHOLDS[0]
            and eval2 < self.opt.TRAIN.STOP_THRESHOLDS[1]
            and eval3 < self.opt.TRAIN.STOP_THRESHOLDS[2]
        )
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                self.register_history(
                    "target_centers", target_centers, self.opt.CLUSTERING.HISTORY_LEN
                )
                self.register_history(
                    "ts_center_dist", center_change, self.opt.CLUSTERING.HISTORY_LEN
                )
                self.register_history(
                    "target_labels", path2label, self.opt.CLUSTERING.HISTORY_LEN
                )

                if (
                    self.clustered_target_samples is not None
                    and self.clustered_target_samples["gt"] is not None
                ):
                    preds = to_onehot(
                        self.clustered_target_samples["label"],
    ...
    def update_labels(self):
        ...
        source_centers = solver_utils.get_centers(
            net,
            source_dataloader,
            self.opt.DATASET.NUM_CLASSES,
            self.opt.CLUSTERING.FEAT_KEY,
    ...
    def construct_categorical_dataloader(self, samples, filtered_classes):
        ...
        dataloader.target_paths = {
            classnames[c]: target_classwise[c]["data"] for c in filtered_classes
        }
    ...
    def compute_iters_per_loop(self, filtered_classes):
        self.iters_per_loop = (
            int(len(self.train_data["categorical"]["loader"]))
    ...
    def update_network(self, filtered_classes):
        ...
        self.train_data["categorical"]["iterator"] = iter(
            self.train_data["categorical"]["loader"]
        )
        ...
        while not stop:
            ...
            source_data, source_gt = source_sample["Img"], source_sample["Label"]
            ...
            if len(filtered_classes) > 0:
                ...
                source_cls_concat = torch.cat(
                    [to_cuda(samples) for samples in source_samples_cls], dim=0
                )
                target_cls_concat = torch.cat(
                    [to_cuda(samples) for samples in target_samples_cls], dim=0
                )
                ...
                cdd_loss = self.cdd.forward(
                    feats_toalign_S, feats_toalign_T, source_nums_cls, target_nums_cls
                )[self.discrepancy_key]
            ...
            if (
                self.opt.TRAIN.LOGGING
                and (update_iters + 1)
                % (max(1, self.iters_per_loop // self.opt.TRAIN.NUM_LOGGING_PER_LOOP))
                == 0
            ):
            ...
            if (
                self.opt.TRAIN.TEST_INTERVAL > 0
                and (update_iters + 1)
                % int(self.opt.TRAIN.TEST_INTERVAL * self.iters_per_loop)
                == 0
            ):
                with torch.no_grad():
                    self.net.module.set_bn_domain(self.bn_domain_map[self.target_name])
                    accu = self.test()
                    print(
                        "Test at (loop %d, iters: %d) with %s: %.4f."
                        % (self.loop, self.iters, self.opt.EVAL_METRIC, accu)
                    )

            if (
                self.opt.TRAIN.SAVE_CKPT_INTERVAL > 0
                and (update_iters + 1)
                % int(self.opt.TRAIN.SAVE_CKPT_INTERVAL * self.iters_per_loop)
                == 0
            ):
                self.save_ckpt()
