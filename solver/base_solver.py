...
class BaseSolver(object):
    def __init__(self, net, dataloader, bn_domain_map={}, resume=None, **kwargs):
        ...
        if resume is not None:
            ...
            print("Resume Training from loop %d, iters %d." % (self.loop, self.iters))
    ...
    def build_optimizer(self):
        opt = self.opt
        param_groups = solver_utils.set_param_groups(
            self.net, dict({"FC": opt.TRAIN.LR_MULT})
        )

        assert opt.TRAIN.OPTIMIZER in [
            "Adam",
            "SGD",
    ...
    def update_lr(self):
        ...
        if self.opt.TRAIN.LR_SCHEDULE == "exp":
            solver_utils.adjust_learning_rate_exp(
                self.base_lr,
                self.optimizer,
                iters,
            ...
            solver_utils.adjust_learning_rate_inv(
                self.base_lr,
                self.optimizer,
                iters,
                self.opt.INV.ALPHA,
            ...
            raise NotImplementedError(
                "Currently don't support the specified \
                    learning rate schedule: %s."
                % self.opt.TRAIN.LR_SCHEDULE
            )
    ...
    def logging(self, loss, accu):
        ...
        loss_str = (
            loss_names
            + loss_values
            + (" source %s: %.4f." % (self.opt.EVAL_METRIC, accu))
        )
        print(loss_str)

    def model_eval(self, preds, gts):
        assert self.opt.EVAL_METRIC in [
            "mean_accu",
            "accuracy",
    ...
    def save_ckpt(self):
        ...
        ckpt_resume = os.path.join(
            save_path, "ckpt_%d_%d.resume" % (self.loop, self.iters)
        )
        ckpt_weights = os.path.join(
            save_path, "ckpt_%d_%d.weights" % (self.loop, self.iters)
        )
        torch.save(
            {
                "loop": self.loop,
        ...
        torch.save(
            {
                "weights": self.net.module.state_dict(),
                "bn_domain_map": self.bn_domain_map,
            },
            ckpt_weights,
        )
    ...
    def register_history(self, key, value, history_len):
        ...
        if len(self.history[key]) > history_len:
            self.history[key] = self.history[key][
                len(self.history[key]) - history_len :
            ]
    ...
    def get_samples(self, data_name):
        ...
        assert (
            "loader" in self.train_data[data_name]
            and "iterator" in self.train_data[data_name]
        )
