...
def filter_samples(samples, threshold=0.05):
    ...
    filtered_data = [
        samples["data"][m] for m in range(mask.size(0)) if mask[m].item() == 1
    ]
    filtered_label = torch.masked_select(samples["label"], mask)
    filtered_gt = (
        torch.masked_select(samples["gt"], mask) if samples["gt"] is not None else None
    )
...
def adjust_learning_rate_RevGrad(
    lr, optimizer, max_iter, cur_iter, alpha=10, beta=0.75
):
