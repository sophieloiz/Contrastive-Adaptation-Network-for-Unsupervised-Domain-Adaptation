...
class CategoricalDataset(Dataset):
    ...
    def initialize(
        ...
        **kwargs
        ...
        for c in self.class_set:
            ...
            self.batch_sizes[self.root][cid] = min(
                batch_size, len(self.data_paths[self.root][cid])
            )
            cid += 1
    ...
    def __getitem__(self, index):
        ...
        inds = random.sample(
            range(len(cur_paths[index])), self.batch_sizes[root][index]
        )
        ...
        data["Label"] = [self.classnames.index(self.class_set[index])] * len(
            data["Img"]
        )
...
class CategoricalSTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for d in ["source", "target"]:
            ...
            inds = random.sample(
                range(len(cur_paths[index])), self.batch_sizes[d][index]
            )
            ...
            data["Label_" + d] = [self.classnames.index(self.class_set[index])] * len(
                data["Img_" + d]
            )
